  ...
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    ...
    >
      <div v-bind:style="`background:${showColor}`" class="colorBtnBorder">
        <strong v-bind:style="`color: #FFF`">A</strong>
      </div>
    </div>
...
.m-colorPicker{
  position: relative; text-align: left; font-size: 12px; display: inline-block;
  ...
  .colorBtn{ width: 100%; 
            ...
            text-align: center;
            color:#fff}
  .colorBtnBorder{padding-left: 3px;padding-right: 3px;width:16px; height: 16px;}
